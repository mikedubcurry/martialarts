...
export default function ViewSessionModal({ session, close }) {
    ...
    return (
        <>
            ...
            <div className='mx-4 w-1/4 p-8 rounded-xl fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white z-50'>
                <div className='flex justify-between items-center'>
                    <h2 className='text-2xl font-bold'>Session Details</h2>
                    <button onClick={() => close()} className='text-2xl font-bold'>X</button>
                </div>
                <div className='mt-4'>
                    <p className='text-xl font-bold capitalize'>{session.discipline}</p>
                    <p className='text-md'>{session.date}</p>
                    <p className='text-md'>{session.start_time} - {session.end_time}</p>
                    <p className='text-md'><Link className="underline capitalize" href={route('gyms.show', session.gym.slug)}>{session.gym.name}</Link></p>
                </div>
                {session.notes?.note || editing && (
                    <div className='mt-4'>
                        <h3 className='text-xl font-bold'>Notes</h3>
                        {editing ? (<textarea className="border" value={newNote} onChange={e => setNewNote(e.target.value)} ></textarea>) : (<p>{session.notes?.note}</p>)}
                    </div>
                )}
                <div className='mt-4'>
